...
namespace Samples.Mvc5.Controllers
{
    public class HomeController : BaseController
    {
        ...
        public ActionResult FetchRouteHits()
        {
            ...
            {
                return Json(new List<string> { ex.Message, ex.StackTrace }, JsonRequestBehavior.AllowGet);
            }
        }
            ...
            using (MiniProfiler.Current.Step("EF Stuff"))
            {
                try
                {
                    using (MiniProfiler.Current.Step("Create Context"))
                        context = new EFContext(MvcApplication.ConnectionString);
                ...
                {
                    return Content($"EF Code First Error: {ex}");
                }
                finally
                {
                    context?.Dispose();
        ...
        public ActionResult ParameterizedSqlWithEnums()
        {
            using (var conn = GetConnection())
            {
                var shouldBeOne = conn.Query<long>("select 1 from dual where :OK = 200", new { System.Net.HttpStatusCode.OK }).Single();
