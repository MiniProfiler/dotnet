...
namespace StackExchange.Profiling.Tests
{
    ...
    public class SkippableMessageBus : IMessageBus
    {
        ...
        public bool QueueMessage(IMessageSinkMessage message)
        {
            ...
            {
                for (var i = 0; i < testFailed.ExceptionTypes.Length; i++)
                {
                    if (testFailed.ExceptionTypes[i] == typeof(SkipTestException).FullName)
                    {
                        DynamicallySkippedTestCount++;
                        return InnerBus.QueueMessage(new TestSkipped(testFailed.Test, testFailed.Messages[i]));
                    }
                }
            }
